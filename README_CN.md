...
逐任务执行并在每个任务后完整审查，或者把整个 epic 交给它处理。两种方式保证相同。

```bash
# 逐任务（每个任务后审查）
/flow-code:work fn-1.1

# 整个 epic（顺序执行，所有任务完成后审查）
/flow-code:work fn-1

# 整个 epic（并行 — 无依赖的任务同时执行）
/flow-code:work fn-1 --parallel
```

所有模式都有：每个任务前重锚定、证据记录、跨模型审查（如果有 rp-cli）。

**并行模式**（Wave-Checkpoint-Wave）：同时为所有就绪任务（无未解决的依赖）spawn worker。每批完成后，运行结构化 **Batch Checkpoint**：聚合结果、集成验证（guard + invariants）、输出 wave 摘要报告，然后规划下一波。新解除阻塞的任务成为下一批。安全因为 `flowctl ready` 只返回所有依赖已解决的任务。

Worker 在单任务内也使用**文件级 Wave 并行** — 当涉及 3+ 文件时，一条消息并行发出所有 Read，在 checkpoint 分析依赖关系，然后一条消息并行发出所有独立 Edit。相比串行文件 I/O 可获得 3-4x 加速。

**审查时机**：RepoPrompt 审查在工作包结束时运行一次 — 如果指定了 `fn-N.M` 则在单个任务后，如果指定了 `fn-N` 则在所有任务后。对于大型 epic 想要更紧密的审查循环，请逐任务执行。
...
| 模式 | 命令 | 何时使用 |
|------|------|---------|
| **交互式** | `/flow-code:work fn-1` | 复杂任务、学习代码库、需要判断、想要干预 |
| **自治式（Ralph）** | `scripts/ralph/ralph.sh` | 明确的 spec、批量实现、过夜运行 |

**启发式**：如果你能写出复选框式的验收标准，就可以用 Ralph。如果不能，你需要的不是循环 — 而是思考。

全自治模式下，在启动 Ralph 前准备 5-10 个计划。详见 [Ralph 模式](#ralph自治模式)。
...
**多模型审查门控**：Ralph 使用 [RepoPrompt](https://repoprompt.com/?atp=KJbuL4)（macOS）或 OpenAI Codex CLI（跨平台）将计划和实现审查发送给*不同的*模型。第二双眼睛捕获自我审查遗漏的盲点。
...
[RepoPrompt](https://repoprompt.com/?atp=KJbuL4) 在 macOS 上提供最佳审查体验。
...
- Python 3.8+
- git
- 可选：[RepoPrompt](https://repoprompt.com/?atp=KJbuL4)（macOS GUI 审查 + 启用 **context-scout** 进行更深层代码库发现。无 RP 时审查通过 Codex 后端工作。）
