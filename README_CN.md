...
```bash
# 全自动：头脑风暴 → 规划 → 实现 → 测试 → 提交 → draft PR
/flow-code:go "添加 OAuth 支持"

# 跳过头脑风暴：规划 → 实现 → 审查 → 关闭
/flow-code:run "添加 OAuth 支持"
...
必需：`git`、`jq`、`gh`（GitHub CLI）、`curl`

可选：`rp-cli`（Layer 2 计划审查）、`codex`（Layer 3 对抗审查）
...
| 起点 | 推荐顺序 |
|------|---------|
| **一键搞定（零操作）** | **`/flow-code:go "想法"`** — 全自动，从头脑风暴到 PR |
...
| 命令 | 功能 |
|------|------|
| `/flow-code:go <想法>` | **全自动**：头脑风暴 → 规划 → 执行 → 审查 → 关闭 → PR（零输入） |
| `/flow-code:run <描述\|fn-N>` | 流水线：规划 → 执行 → 审查 → 关闭（跳过头脑风暴） |
| `/flow-code:brainstorm <想法>` | 探索和压力测试想法（`--auto` 开启 AI 自问自答模式） |
| `/flow-code:plan <想法>` | 研究代码库，创建带依赖排序任务的 epic |
| `/flow-code:work <id\|文件>` | 执行 epic、任务或 spec 文件，每个任务前重锚定 |
| `/flow-code:interview <id>` | 深度访谈以完善 spec（40+ 问题） |
...
| `/flow-code:debug` | 系统化调试：根因调查 → 模式分析 → 假设 → 修复 |
...
| `/flow-code:retro` | 事后复盘：哪些有效、哪些无效、经验 → 记忆 |
| `/flow-code:django` | Django 模式：架构、DRF、安全、测试、部署验证 |
| `/flow-code:qa` | 浏览器自动化视觉 QA 测试 |
| `/flow-code:design-review` | 浏览器自动化设计审查 |
| `/flow-code:autoplan` | 多视角自动审查（CEO、工程、设计、DX） |
...
| `/flow-code:brainstorm` | `--auto`（AI 自问自答，零人工输入） |
| `/flow-code:plan` | `--research=rp\|grep`、`--depth=short\|standard\|deep`、`--review=rp\|codex\|export\|none`、`--plan-only`、`--interactive` |
| `/flow-code:work` | `--branch=current\|new\|worktree`、`--review=rp\|codex\|export\|none`、`--no-review`、`--interactive`、`--tdd`、`--no-pr` |
