...
给 flowctl 加文件 frecency 追踪：

```bash
# Worker 完成时自动记录修改过的文件
flowctl frecency record --task fn-1.1 --files "src/auth.rs,src/middleware.rs"

# 规划时查询：这类任务通常改哪些文件？
flowctl frecency suggest --keywords "auth,login" --json
# → {"files": [{"path": "src/auth.rs", "score": 850}, ...]}
```

**集成点**：
- Worker Phase 7（提交）后自动记录修改文件
- Plan 阶段的 repo-scout 用 frecency 数据补充文件发现
- `flowctl task create --files` 自动建议基于 frecency
...
4. 如果仍无结果 → 用 flowctl frecency suggest 查历史
