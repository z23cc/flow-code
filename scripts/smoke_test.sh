...
cp "$PLUGIN_ROOT/scripts/flowctl.sh" scripts/flowctl.sh
cp -r "$PLUGIN_ROOT/scripts/flowctl" scripts/flowctl
chmod +x scripts/flowctl.sh

scripts/flowctl.sh init --json >/dev/null
...
init_result="$(scripts/flowctl.sh init --json)"
...
init_upgrade="$(scripts/flowctl.sh init --json)"
...
memory_val="$(scripts/flowctl.sh config get memory.enabled --json | "$PYTHON_BIN" -c 'import json,sys; print(json.load(sys.stdin).get("value"))')"
...
plansync_val="$(scripts/flowctl.sh config get planSync.enabled --json | "$PYTHON_BIN" -c 'import json,sys; print(json.load(sys.stdin).get("value"))')"
...
scripts/flowctl.sh config set memory.enabled false --json >/dev/null

echo -e "${YELLOW}--- next: plan/work/none + priority ---${NC}"
# Capture epic ID from create output (fn-N-xxx format)
EPIC1_JSON="$(scripts/flowctl.sh epic create --title "Epic One" --json)"
EPIC1="$(echo "$EPIC1_JSON" | "$PYTHON_BIN" -c 'import json,sys; print(json.load(sys.stdin)["id"])')"
scripts/flowctl.sh task create --epic "$EPIC1" --title "Low pri" --priority 5 --json >/dev/null
scripts/flowctl.sh task create --epic "$EPIC1" --title "High pri" --priority 1 --json >/dev/null

plan_json="$(scripts/flowctl.sh next --require-plan-review --json)"
...
scripts/flowctl.sh epic set-plan-review-status "$EPIC1" --status ship --json >/dev/null
work_json="$(scripts/flowctl.sh next --json)"
...
scripts/flowctl.sh start "${EPIC1}.2" --json >/dev/null
scripts/flowctl.sh done "${EPIC1}.2" --summary-file "$TEST_DIR/summary.md" --evidence-json "$TEST_DIR/evidence.json" --json >/dev/null
scripts/flowctl.sh start "${EPIC1}.1" --json >/dev/null
scripts/flowctl.sh done "${EPIC1}.1" --summary-file "$TEST_DIR/summary.md" --evidence-json "$TEST_DIR/evidence.json" --json >/dev/null
none_json="$(scripts/flowctl.sh next --json)"
...
set +e
next_result="$(scripts/flowctl.sh next --json 2>&1)"
next_rc=$?
set -e
...
set +e
list_result="$(scripts/flowctl.sh list --json 2>&1)"
list_rc=$?
set -e
...
set +e
ready_result="$(scripts/flowctl.sh ready --epic "$EPIC1" --json 2>&1)"
ready_rc=$?
set -e
...
set +e
show_result="$(scripts/flowctl.sh show "$EPIC1" --json 2>&1)"
show_rc=$?
set -e
...
set +e
validate_result="$(scripts/flowctl.sh validate --epic "$EPIC1" --json 2>&1)"
validate_rc=$?
set -e
...
PY
show_json="$(scripts/flowctl.sh show "$EPIC1" --json)"
...
scripts/flowctl.sh epic set-branch "$EPIC1" --branch "${EPIC1}-epic" --json >/dev/null
show_json="$(scripts/flowctl.sh show "$EPIC1" --json)"
...
RENAME_EPIC_JSON="$(scripts/flowctl.sh epic create --title "Old Title" --json)"
RENAME_EPIC="$(echo "$RENAME_EPIC_JSON" | "$PYTHON_BIN" -c 'import json,sys; print(json.load(sys.stdin)["id"])')"
scripts/flowctl.sh task create --epic "$RENAME_EPIC" --title "First task" --json >/dev/null
scripts/flowctl.sh task create --epic "$RENAME_EPIC" --title "Second task" --json >/dev/null
# Add task dependency within epic
scripts/flowctl.sh dep add "${RENAME_EPIC}.2" "${RENAME_EPIC}.1" --json >/dev/null

# Rename epic
rename_result="$(scripts/flowctl.sh epic set-title "$RENAME_EPIC" --title "New Shiny Title" --json)"
...
show_json="$(scripts/flowctl.sh show "$NEW_EPIC" --json)"
...
DEP_EPIC_JSON="$(scripts/flowctl.sh epic create --title "Depends on renamed" --json)"
DEP_EPIC="$(echo "$DEP_EPIC_JSON" | "$PYTHON_BIN" -c 'import json,sys; print(json.load(sys.stdin)["id"])')"
scripts/flowctl.sh epic add-dep "$DEP_EPIC" "$NEW_EPIC" --json >/dev/null
# Rename the dependency
rename2_result="$(scripts/flowctl.sh epic set-title "$NEW_EPIC" --title "Final Title" --json)"
...
EPIC2_JSON="$(scripts/flowctl.sh epic create --title "Epic Two" --json)"
EPIC2="$(echo "$EPIC2_JSON" | "$PYTHON_BIN" -c 'import json,sys; print(json.load(sys.stdin)["id"])')"
scripts/flowctl.sh task create --epic "$EPIC2" --title "Block me" --json >/dev/null
scripts/flowctl.sh task create --epic "$EPIC2" --title "Other" --json >/dev/null
printf "Blocked by test\n" > "$TEST_DIR/reason.md"
scripts/flowctl.sh block "${EPIC2}.1" --reason-file "$TEST_DIR/reason.md" --json >/dev/null
scripts/flowctl.sh validate --epic "$EPIC2" --json >/dev/null
echo -e "${GREEN}✓${NC} validate allows blocked"
PASS=$((PASS + 1))

set +e
scripts/flowctl.sh epic close "$EPIC2" --json >/dev/null
rc=$?
set -e
if [[ "$rc" -ne 0 ]]; then
...
scripts/flowctl.sh start "${EPIC2}.1" --force --json >/dev/null
scripts/flowctl.sh done "${EPIC2}.1" --summary-file "$TEST_DIR/summary.md" --evidence-json "$TEST_DIR/evidence.json" --json >/dev/null
scripts/flowctl.sh start "${EPIC2}.2" --json >/dev/null
scripts/flowctl.sh done "${EPIC2}.2" --summary-file "$TEST_DIR/summary.md" --evidence-json "$TEST_DIR/evidence.json" --json >/dev/null
scripts/flowctl.sh epic close "$EPIC2" --json >/dev/null
echo -e "${GREEN}✓${NC} epic close succeeds when done"
PASS=$((PASS + 1))

echo -e "${YELLOW}--- config set/get ---${NC}"
scripts/flowctl.sh config set memory.enabled true --json >/dev/null
config_json="$(scripts/flowctl.sh config get memory.enabled --json)"
...
scripts/flowctl.sh config set planSync.enabled true --json >/dev/null
config_json="$(scripts/flowctl.sh config get planSync.enabled --json)"
"$PYTHON_BIN" - <<'PY' "$config_json"
import json, sys
data = json.loads(sys.argv[1])
...
scripts/flowctl.sh config set planSync.enabled false --json >/dev/null
config_json="$(scripts/flowctl.sh config get planSync.enabled --json)"
"$PYTHON_BIN" - <<'PY' "$config_json"
import json, sys
data = json.loads(sys.argv[1])
...
# Test 1: gap add
gap_add_result="$(scripts/flowctl.sh gap add --epic "$EPIC1" --capability "Missing auth check" --priority required --source flow-gap-analyst --json)"
...
gap_dup_result="$(scripts/flowctl.sh gap add --epic "$EPIC1" --capability "Missing auth check" --priority required --json)"
...
scripts/flowctl.sh gap add --epic "$EPIC1" --capability "Optional caching" --priority nice-to-have --json >/dev/null

# Test 4: gap list
gap_list_count="$(scripts/flowctl.sh gap list --epic "$EPIC1" --json | "$PYTHON_BIN" -c 'import json,sys; print(json.load(sys.stdin).get("count", 0))')"
...
gap_open_count="$(scripts/flowctl.sh gap list --epic "$EPIC1" --status open --json | "$PYTHON_BIN" -c 'import json,sys; print(json.load(sys.stdin).get("count", 0))')"
...
if ! scripts/flowctl.sh gap check --epic "$EPIC1" --json >/dev/null 2>&1; then
...
gap_check_gate="$(scripts/flowctl.sh gap check --epic "$EPIC1" --json 2>/dev/null || true)"
...
# Test 8: gap resolve
gap_resolve_result="$(scripts/flowctl.sh gap resolve --epic "$EPIC1" --capability "Missing auth check" --evidence "Added in auth.py:42" --json)"
...
gap_resolve_dup="$(scripts/flowctl.sh gap resolve --epic "$EPIC1" --capability "Missing auth check" --evidence "duplicate" --json)"
...
gap_pass_gate="$(scripts/flowctl.sh gap check --epic "$EPIC1" --json)"
...
scripts/flowctl.sh config set memory.enabled true --json >/dev/null
scripts/flowctl.sh memory init --json >/dev/null
...
add_result="$(scripts/flowctl.sh memory add pitfall "Test pitfall entry" --json)"
...
scripts/flowctl.sh memory add convention "Test convention" --json >/dev/null
scripts/flowctl.sh memory add decision "Test decision" --json >/dev/null
list_json="$(scripts/flowctl.sh memory list --json)"
...
path.write_text(json.dumps(data, indent=2, sort_keys=True) + "\n")
PY
scripts/flowctl.sh validate --all --json >/dev/null
echo -e "${GREEN}✓${NC} schema v1 validate"
PASS=$((PASS + 1))

echo -e "${YELLOW}--- codex commands ---${NC}"
# Test codex check (may or may not have codex installed)
codex_check_json="$(scripts/flowctl.sh codex check --json 2>/dev/null || echo '{"success":true}')"
...
set +e
scripts/flowctl.sh codex impl-review --help >/dev/null 2>&1
rc=$?
set -e
if [[ "$rc" -eq 0 ]]; then
...
set +e
scripts/flowctl.sh codex plan-review --help >/dev/null 2>&1
rc=$?
set -e
if [[ "$rc" -eq 0 ]]; then
...
codex_available="$(scripts/flowctl.sh codex check --json 2>/dev/null | "$PYTHON_BIN" -c "import sys,json; print(json.load(sys.stdin).get('available', False))" 2>/dev/null || echo "False")"
if [[ "$codex_available" == "True" ]]; then
  # Create a simple epic + task for testing
  EPIC3_JSON="$(scripts/flowctl.sh epic create --title "Codex test epic" --json)"
  EPIC3="$(echo "$EPIC3_JSON" | "$PYTHON_BIN" -c 'import json,sys; print(json.load(sys.stdin)["id"])')"
  scripts/flowctl.sh task create --epic "$EPIC3" --title "Test task" --json >/dev/null
...
EOF
  ...
  set +e
  plan_result="$(scripts/flowctl.sh codex plan-review "$EPIC3" --files "src/hello.py" --base main --receipt "$TEST_DIR/plan-receipt.json" --json 2>&1)"
  plan_rc=$?
  set -e
...
EOF
  ...
  set +e
  impl_result="$(scripts/flowctl.sh codex impl-review "${EPIC3}.1" --base HEAD~1 --receipt "$TEST_DIR/impl-receipt.json" --json 2>&1)"
  impl_rc=$?
  set -e
...
DEP_BASE_JSON="$(scripts/flowctl.sh epic create --title "Dep base" --json)"
DEP_BASE_ID="$(echo "$DEP_BASE_JSON" | "$PYTHON_BIN" -c 'import json,sys; print(json.load(sys.stdin)["id"])')"
scripts/flowctl.sh task create --epic "$DEP_BASE_ID" --title "Base task" --json >/dev/null
DEP_CHILD_JSON="$(scripts/flowctl.sh epic create --title "Dep child" --json)"
...
blocked_json="$(scripts/flowctl.sh next --epics-file "$TEST_DIR/epics.json" --json)"
...
cd "$TEST_DIR/repo"
STDIN_EPIC_JSON="$(scripts/flowctl.sh epic create --title "Stdin test" --json)"
STDIN_EPIC="$(echo "$STDIN_EPIC_JSON" | "$PYTHON_BIN" -c 'import json,sys; print(json.load(sys.stdin)["id"])')"
# Test epic set-plan with stdin
scripts/flowctl.sh epic set-plan "$STDIN_EPIC" --file - --json <<'EOF'
# Stdin Test Plan
...
spec_content="$(scripts/flowctl.sh cat "$STDIN_EPIC")"
...
scripts/flowctl.sh task create --epic "$STDIN_EPIC" --title "Set-spec test" --json >/dev/null
...
scripts/flowctl.sh task set-spec "$SETSPEC_TASK" --description "$TEST_DIR/desc.md" --acceptance "$TEST_DIR/acc.md" --json >/dev/null
# Verify both sections were written
task_spec="$(scripts/flowctl.sh cat "$SETSPEC_TASK")"
...
scripts/flowctl.sh task create --epic "$STDIN_EPIC" --title "Full replacement test" --json >/dev/null
...
FULLSPEC
scripts/flowctl.sh task set-spec "$FULLREPLACE_TASK" --file "$TEST_DIR/full_spec.md" --json >/dev/null
# Verify full replacement
full_spec="$(scripts/flowctl.sh cat "$FULLREPLACE_TASK")"
...
scripts/flowctl.sh task create --epic "$STDIN_EPIC" --title "Stdin replacement test" --json >/dev/null
STDIN_REPLACE_TASK="${STDIN_EPIC}.3"
# Full replacement via stdin
scripts/flowctl.sh task set-spec "$STDIN_REPLACE_TASK" --file - --json <<'EOF'
...
stdin_spec="$(scripts/flowctl.sh cat "$STDIN_REPLACE_TASK")"
...
# Save checkpoint
scripts/flowctl.sh checkpoint save --epic "$STDIN_EPIC" --json >/dev/null
...
scripts/flowctl.sh checkpoint restore --epic "$STDIN_EPIC" --json >/dev/null
# Verify original content restored
restored_spec="$(scripts/flowctl.sh cat "$STDIN_EPIC")"
echo "$restored_spec" | grep -q "Testing stdin support" || { echo "checkpoint restore failed"; FAIL=$((FAIL + 1)); }
# Delete checkpoint
scripts/flowctl.sh checkpoint delete --epic "$STDIN_EPIC" --json >/dev/null
...
DUR_EPIC_JSON="$(scripts/flowctl.sh epic create --title "Duration test" --json)"
DUR_EPIC="$("$PYTHON_BIN" -c "import json,sys; print(json.loads(sys.argv[1])['id'])" "$DUR_EPIC_JSON")"
scripts/flowctl.sh task create --epic "$DUR_EPIC" --title "Timed task" --json > /dev/null
scripts/flowctl.sh start "${DUR_EPIC}.1" --json > /dev/null
sleep 1
result="$(scripts/flowctl.sh done "${DUR_EPIC}.1" --summary "done" --evidence '{"commits":[],"tests":[],"prs":[]}' --json)"
...
SPEC="$(scripts/flowctl.sh cat "${DUR_EPIC}.1")"
...
WS_EPIC_JSON="$(scripts/flowctl.sh epic create --title "Workspace test" --json)"
WS_EPIC="$("$PYTHON_BIN" -c "import json,sys; print(json.loads(sys.argv[1])['id'])" "$WS_EPIC_JSON")"
scripts/flowctl.sh task create --epic "$WS_EPIC" --title "WS task" --json > /dev/null
scripts/flowctl.sh start "${WS_EPIC}.1" --json > /dev/null
...
result="$(scripts/flowctl.sh done "${WS_EPIC}.1" --summary "done" --evidence "$WS_EVIDENCE" --json)"
"$PYTHON_BIN" - "$result" <<'PY'
import json, sys
data = json.loads(sys.argv[1])
...
WS_SPEC="$(scripts/flowctl.sh cat "${WS_EPIC}.1")"
...
scripts/flowctl.sh task reset "${WS_EPIC}.1" --json > /dev/null
scripts/flowctl.sh start "${WS_EPIC}.1" --force --json > /dev/null
BAD_EVIDENCE='{"commits":[],"tests":[],"prs":[],"workspace_changes":{"baseline_rev":"aaa"}}'
result="$(scripts/flowctl.sh done "${WS_EPIC}.1" --summary "done" --evidence "$BAD_EVIDENCE" --json)"
"$PYTHON_BIN" - "$result" <<'PY'
import json, sys
data = json.loads(sys.argv[1])
assert data.get("status") == "done"
...
FO_EPIC_JSON="$(scripts/flowctl.sh epic create --title "Files test" --json)"
FO_EPIC="$("$PYTHON_BIN" -c "import json,sys; print(json.loads(sys.argv[1])['id'])" "$FO_EPIC_JSON")"
scripts/flowctl.sh task create --epic "$FO_EPIC" --title "T1" --files "src/auth.ts,src/middleware.ts" --json > /dev/null
scripts/flowctl.sh task create --epic "$FO_EPIC" --title "T2" --files "src/routes.ts" --json > /dev/null
scripts/flowctl.sh task create --epic "$FO_EPIC" --title "T3" --files "src/auth.ts" --json > /dev/null

# Test 1: files stored in task JSON
result="$(scripts/flowctl.sh show "${FO_EPIC}.1" --json)"
"$PYTHON_BIN" - "$result" <<'PY'
import json, sys
...
result="$(scripts/flowctl.sh files --epic "$FO_EPIC" --json)"
"$PYTHON_BIN" - "$result" <<'PY'
import json, sys
d = json.loads(sys.argv[1])
...
scripts/flowctl.sh config set memory.enabled true --json > /dev/null
scripts/flowctl.sh memory init --json > /dev/null
scripts/flowctl.sh memory add pitfall "Test pitfall for verify" --json > /dev/null

# Test 1: memory verify updates last_verified
result="$(scripts/flowctl.sh memory verify 1 --json)"
"$PYTHON_BIN" - "$result" <<'PY'
import json, sys
data = json.loads(sys.argv[1])
...
result="$(scripts/flowctl.sh memory list --json)"
"$PYTHON_BIN" - "$result" <<'PY'
import json, sys
data = json.loads(sys.argv[1])
entry = data["index"][0]
...
EPC_EPIC_JSON="$(scripts/flowctl.sh epic create --title "Retro prompt test" --json)"
EPC_EPIC="$("$PYTHON_BIN" -c "import json,sys; print(json.loads(sys.argv[1])['id'])" "$EPC_EPIC_JSON")"
scripts/flowctl.sh task create --epic "$EPC_EPIC" --title "Done task" --json > /dev/null
scripts/flowctl.sh start "${EPC_EPIC}.1" --json > /dev/null
scripts/flowctl.sh done "${EPC_EPIC}.1" --summary "ok" --evidence '{"commits":[],"tests":[],"prs":[]}' --json > /dev/null
result="$(scripts/flowctl.sh epic close "$EPC_EPIC" --json)"
"$PYTHON_BIN" - "$result" <<'PY'
import json, sys
data = json.loads(sys.argv[1])
...
RST_EPIC_JSON="$(scripts/flowctl.sh epic create --title "Restart test" --json)"
RST_EPIC="$("$PYTHON_BIN" -c "import json,sys; print(json.loads(sys.argv[1])['id'])" "$RST_EPIC_JSON")"
scripts/flowctl.sh task create --epic "$RST_EPIC" --title "Task 1" --json > /dev/null
scripts/flowctl.sh task create --epic "$RST_EPIC" --title "Task 2" --deps "${RST_EPIC}.1" --json > /dev/null
scripts/flowctl.sh task create --epic "$RST_EPIC" --title "Task 3" --deps "${RST_EPIC}.2" --json > /dev/null

# Complete tasks 1, 2, 3
scripts/flowctl.sh start "${RST_EPIC}.1" --json > /dev/null
scripts/flowctl.sh done "${RST_EPIC}.1" --summary "done" --evidence '{"commits":[],"tests":[],"prs":[]}' --json > /dev/null
scripts/flowctl.sh start "${RST_EPIC}.2" --json > /dev/null
scripts/flowctl.sh done "${RST_EPIC}.2" --summary "done" --evidence '{"commits":[],"tests":[],"prs":[]}' --json > /dev/null
scripts/flowctl.sh start "${RST_EPIC}.3" --json > /dev/null
scripts/flowctl.sh done "${RST_EPIC}.3" --summary "done" --evidence '{"commits":[],"tests":[],"prs":[]}' --json > /dev/null

# Test 1: restart --dry-run shows what would be reset
result="$(scripts/flowctl.sh restart "${RST_EPIC}.1" --dry-run --json)"
...
result="$(scripts/flowctl.sh restart "${RST_EPIC}.1" --json)"
"$PYTHON_BIN" - "$result" "$RST_EPIC" <<'PY'
import json, sys
data = json.loads(sys.argv[1])
ep = sys.argv[2]
...
result="$(scripts/flowctl.sh show "${RST_EPIC}.1" --json)"
"$PYTHON_BIN" - "$result" <<'PY'
import json, sys
data = json.loads(sys.argv[1])
...
scripts/flowctl.sh start "${RST_EPIC}.1" --json > /dev/null
set +e
result="$(scripts/flowctl.sh restart "${RST_EPIC}.1" --json 2>&1)"
rc=$?
set -e
...
result="$(scripts/flowctl.sh restart "${RST_EPIC}.1" --force --json)"
"$PYTHON_BIN" - "$result" "$RST_EPIC" <<'PY'
import json, sys
data = json.loads(sys.argv[1])
ep = sys.argv[2]
...
result="$(scripts/flowctl.sh review-backend --compare "$TEST_DIR/receipt-codex.json,$TEST_DIR/receipt-rp.json" --json)"
"$PYTHON_BIN" - "$result" <<'PY'
import json, sys
data = json.loads(sys.argv[1])
...
result="$(scripts/flowctl.sh review-backend --compare "$TEST_DIR/receipt-codex.json,$TEST_DIR/receipt-conflict.json" --json)"
"$PYTHON_BIN" - "$result" <<'PY'
import json, sys
data = json.loads(sys.argv[1])
...
RR_EPIC_JSON="$(scripts/flowctl.sh epic create --title "Receipt test" --json)"
RR_EPIC="$("$PYTHON_BIN" -c "import json,sys; print(json.loads(sys.argv[1])['id'])" "$RR_EPIC_JSON")"
scripts/flowctl.sh task create --epic "$RR_EPIC" --title "Task with review" --json > /dev/null
scripts/flowctl.sh start "${RR_EPIC}.1" --json > /dev/null
...
scripts/flowctl.sh done "${RR_EPIC}.1" --summary "done" --evidence "$RR_EVIDENCE" --json > /dev/null
...
result="$(scripts/flowctl.sh review-backend --epic "$RR_EPIC" --json)"
"$PYTHON_BIN" - "$result" <<'PY'
import json, sys
data = json.loads(sys.argv[1])
...
DOM_EPIC_JSON="$(scripts/flowctl.sh epic create --title "Domain test" --json)"
DOM_EPIC="$("$PYTHON_BIN" -c "import json,sys; print(json.loads(sys.argv[1])['id'])" "$DOM_EPIC_JSON")"
scripts/flowctl.sh task create --epic "$DOM_EPIC" --title "Build API" --domain backend --json > /dev/null
scripts/flowctl.sh task create --epic "$DOM_EPIC" --title "Build UI" --domain frontend --json > /dev/null
scripts/flowctl.sh task create --epic "$DOM_EPIC" --title "No domain" --json > /dev/null

# Test 1: domain stored in task JSON
result="$(scripts/flowctl.sh show "${DOM_EPIC}.1" --json)"
"$PYTHON_BIN" - "$result" <<'PY'
import json, sys
data = json.loads(sys.argv[1])
...
result="$(scripts/flowctl.sh show "${DOM_EPIC}.3" --json)"
"$PYTHON_BIN" - "$result" <<'PY'
import json, sys
data = json.loads(sys.argv[1])
...
result="$(scripts/flowctl.sh tasks --epic "$DOM_EPIC" --domain backend --json)"
"$PYTHON_BIN" - "$result" <<'PY'
import json, sys
data = json.loads(sys.argv[1])
...
result="$(scripts/flowctl.sh tasks --epic "$DOM_EPIC" --json)"
"$PYTHON_BIN" - "$result" <<'PY'
import json, sys
data = json.loads(sys.argv[1])
...
ARC_EPIC_JSON="$(scripts/flowctl.sh epic create --title "Archive me" --json)"
ARC_EPIC="$("$PYTHON_BIN" -c "import json,sys; print(json.loads(sys.argv[1])['id'])" "$ARC_EPIC_JSON")"
scripts/flowctl.sh task create --epic "$ARC_EPIC" --title "Done task" --json > /dev/null
scripts/flowctl.sh start "${ARC_EPIC}.1" --json > /dev/null
scripts/flowctl.sh done "${ARC_EPIC}.1" --summary "ok" --evidence '{"commits":[],"tests":[],"prs":[]}' --json > /dev/null
scripts/flowctl.sh epic close "$ARC_EPIC" --json > /dev/null

# Test 1: archive moves files
result="$(scripts/flowctl.sh epic archive "$ARC_EPIC" --json)"
...
result="$(scripts/flowctl.sh epics --json)"
"$PYTHON_BIN" - "$result" "$ARC_EPIC" <<'PY'
import json, sys
data = json.loads(sys.argv[1])
ep = sys.argv[2]
...
CLEAN_EP1_JSON="$(scripts/flowctl.sh epic create --title "Clean1" --json)"
CLEAN_EP1="$("$PYTHON_BIN" -c "import json,sys; print(json.loads(sys.argv[1])['id'])" "$CLEAN_EP1_JSON")"
scripts/flowctl.sh task create --epic "$CLEAN_EP1" --title "T1" --json > /dev/null
scripts/flowctl.sh start "${CLEAN_EP1}.1" --json > /dev/null
scripts/flowctl.sh done "${CLEAN_EP1}.1" --summary "ok" --evidence '{"commits":[],"tests":[],"prs":[]}' --json > /dev/null
scripts/flowctl.sh epic close "$CLEAN_EP1" --json > /dev/null

result="$(scripts/flowctl.sh epic clean --json)"
"$PYTHON_BIN" - "$result" <<'PY'
import json, sys
data = json.loads(sys.argv[1])
