...
```bash
"${PLUGIN_ROOT}/scripts/flowctl.py" init --json
```
...
CURRENT_BACKEND=$("${PLUGIN_ROOT}/scripts/flowctl.py" config get review.backend --json 2>/dev/null | jq -r '.value // empty')
CURRENT_MEMORY=$("${PLUGIN_ROOT}/scripts/flowctl.py" config get memory.enabled --json 2>/dev/null | jq -r '.value // empty')
CURRENT_PLANSYNC=$("${PLUGIN_ROOT}/scripts/flowctl.py" config get planSync.enabled --json 2>/dev/null | jq -r '.value // empty')
CURRENT_CROSSEPIC=$("${PLUGIN_ROOT}/scripts/flowctl.py" config get planSync.crossEpic --json 2>/dev/null | jq -r '.value // empty')
CURRENT_GITHUB_SCOUT=$("${PLUGIN_ROOT}/scripts/flowctl.py" config get scouts.github --json 2>/dev/null | jq -r '.value // empty')
```
...
```json
{
  ...
  "options": [
    {"label": "CLAUDE.md only", "description": "Add flow-code section to CLAUDE.md"},
...
- If "Yes": `"${PLUGIN_ROOT}/scripts/flowctl.py" config set memory.enabled true --json`
- If "No": `"${PLUGIN_ROOT}/scripts/flowctl.py" config set memory.enabled false --json`

**Plan-Sync** (if question was asked):
- If "Yes": `"${PLUGIN_ROOT}/scripts/flowctl.py" config set planSync.enabled true --json`
- If "No": `"${PLUGIN_ROOT}/scripts/flowctl.py" config set planSync.enabled false --json`

**Plan-Sync cross-epic** (if question was asked):
- If "Yes": `"${PLUGIN_ROOT}/scripts/flowctl.py" config set planSync.crossEpic true --json`
- If "No": `"${PLUGIN_ROOT}/scripts/flowctl.py" config set planSync.crossEpic false --json`

**GitHub Scout** (if question was asked):
- If "Yes": `"${PLUGIN_ROOT}/scripts/flowctl.py" config set scouts.github true --json`
- If "No": `"${PLUGIN_ROOT}/scripts/flowctl.py" config set scouts.github false --json`
...
"${PLUGIN_ROOT}/scripts/flowctl.py" config set review.backend "$REVIEW_BACKEND" --json
```
